...
import { ServerSteel } from "../entities/ServerSteel";
...
export class ServerGameScene extends GameScene {
  ...
  // Track server-managed entities
  serverBullets: ServerBullet[] = [];
  steelResources: ServerSteel[] = [];
  ...
  // Create a steel resource at the specified location
  createSteel(x: number, y: number, value: number): ServerSteel {
    // Generate a unique ID for the steel resource
    const id = `steel_${Date.now()}_${Math.floor(Math.random() * 1000)}`;

    // Create the steel resource
    const steel = new ServerSteel(this, x, y, value);
    this.steelResources.push(steel);

    // Add to room state
    this.room.state.map.steelResources.set(id, steel.schema);

    console.log(`Created steel resource (${value} units) at (${x}, ${y})`);
    return steel;
  }
  ...
  update(time: number, delta: number): void {
    ...
    this.players.forEach(tank => {
      ...
      // Check for collisions with steel resources
      this.steelResources.forEach((steel, i) => {
        // Calculate distance between tank and steel
        const distance = Phaser.Math.Distance.Between(
          tank.x, tank.y,
          steel.x, steel.y
        );

        // If close enough, collect the steel
        if (distance < PHYSICS.STEEL_PICKUP_RANGE) {
          tank.pickupSteel(steel);

          // Remove from steel resources array and room state
          this.steelResources.splice(i, 1);
          this.room.state.map.steelResources.delete(`steel_${steel.schema.createdAt}`);
        }
      });
    });
  ...
  override destroyWall(wall: MatterJS.Body) {
    ...
    this.createSteel(wallTilePos.x, wallTilePos.y, PHYSICS.STEEL_FROM_WALL);
  }
}
